...
namespace DomainModel.jqGrid
{
    public static class JqGridExtensions
    {
        ...
        private static IQueryable<T> ListAddSearchQuery<T>(IQueryable<T> baseList, string searchQuery, IEnumerable<string> searchColumns)
        {
            ...
            const string strpredicateFormat = "{0}.ToString().ToLower().Contains(@0)";
            ...
            var filteredList = baseList.Where(searchExpression.ToString(), searchQuery.ToLower());
...
public static class stringExtensions
{
    ...
    {
        int index = s.IndexOf(value, StringComparison.CurrentCultureIgnoreCase); 
